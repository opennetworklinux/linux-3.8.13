...
#define PROJECT_NAME                      accton_as5710_54x_

#define LOCAL_DEBUG                       0

static struct accton_as5710_54x_fan  *fan_data = NULL;

struct accton_as5710_54x_fan {
    struct device   *dev;
...
/*******************/
static void accton_as5710_54x_fan_update_device(struct device *dev);
static int accton_as5710_54x_fan_read_value(u8 reg);
static int accton_as5710_54x_fan_write_value(u8 reg, u8 value);
...
static struct attribute *accton_as5710_54x_fan_attributes[] = {
             ...
             char *buf)
{
    ...
    accton_as5710_54x_fan_update_device(dev);
...
static const struct attribute_group accton_as5710_54x_fan_group = {
    .attrs = accton_as5710_54x_fan_attributes,
};
...
static void accton_as5710_54x_fan_update_device(struct device *dev)
{
    ...
    if (LOCAL_DEBUG)        
        printk ("Starting accton_as5710_54x_fan update \n");    
    ...
    fault      = accton_as5710_54x_fan_read_value(CPLD_REG_FAN_STATUS_OFFSET);
    r_fault    = accton_as5710_54x_fan_read_value(CPLD_REG_FANR_STATUS_OFFSET);    
    direction  = accton_as5710_54x_fan_read_value(CPLD_REG_FAN_DIRECTION_OFFSET);
    ctrl_speed = accton_as5710_54x_fan_read_value(CPLD_REG_FAN_PWM_CYCLE_OFFSET);
    ...
    for (i=0; i<FAN_MAX_NUMBER; i++)
    {
        ...
        /* fan speed 
         */
        speed      = accton_as5710_54x_fan_read_value(fan_speed_reg[i]);
        r_speed    = accton_as5710_54x_fan_read_value(fanr_speed_reg[i]);
...
static int __init accton_as5710_54x_fan_init(void)
{
    int status = 0;

    fan_data = kzalloc(sizeof(struct accton_as5710_54x_fan), GFP_KERNEL);
    if (!fan_data) {
        status = -ENOMEM;
        goto exit;
    }
    ...
    status = sysfs_create_group(&fan_data->dev->kobj, &accton_as5710_54x_fan_group);
    if (status) {
        goto exit_unregister;
    }

    dev_info(fan_data->dev, "accton_as5710_54x_fan\n");
...
static void __exit accton_as5710_54x_fan_exit(void)
{
    sysfs_remove_group(&fan_data->dev->kobj, &accton_as5710_54x_fan_group);
    hwmon_device_unregister(fan_data->dev);
    kfree(fan_data);
}

MODULE_AUTHOR("Brandon Chuang <brandon_chuang@accton.com.tw>");
MODULE_DESCRIPTION("accton_as5710_54x_fan driver");
MODULE_LICENSE("GPL");

module_init(accton_as5710_54x_fan_init);
module_exit(accton_as5710_54x_fan_exit);
